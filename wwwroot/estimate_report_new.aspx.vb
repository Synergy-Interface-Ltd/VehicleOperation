...
Imports System.IO
Partial Class estimate_report_new
    ...
    Protected Sub GridView1_SelectedIndexEdit(ByVal sender As Object, ByVal e As GridViewEditEventArgs) Handles GridView1.RowEditing
        ...
        rsb1.Append("window.open('estimate.aspx?url=" & GridView1.Rows(rowIndex).Cells(2).Text & "','Edit_estimate');")
        rsb1.Append("</scri")
        rsb1.Append("pt>")
    ...
    Protected Sub GridView1_RowCommand(ByVal sender As Object, ByVal e As GridViewCommandEventArgs)
        If e.CommandName = "Select" OrElse e.CommandName = "Print" Then
            Dim commandText As String = e.CommandName
            Dim rowIndex As Integer = Convert.ToInt32(e.CommandArgument)
            If commandText = "Select" Then
                Dim rsb1 As StringBuilder
                rsb1 = New StringBuilder()
                rsb1.Append("<script language=""JavaScript"">")
                rsb1.Append("window.open('estimate_new.aspx?url=" & GridView1.Rows(rowIndex).Cells(2).Text & "','Edit_estimate');")
                rsb1.Append("</scri")
                rsb1.Append("pt>")
                Page.RegisterStartupScript("rtest1", rsb1.ToString())
                If GridView1.Rows.Count > 0 Then
            ...
            Else
                ...
                rsb1 = New StringBuilder()
                rsb1.Append("<script language=""JavaScript"">")
                rsb1.Append("window.open('estimate_bill.aspx?url=" & GridView1.Rows(rowIndex).Cells(2).Text & "','Edit_estimate');")
                rsb1.Append("</scri")
                rsb1.Append("pt>")
                Page.RegisterStartupScript("rtest1", rsb1.ToString())
                If GridView1.Rows.Count > 0 Then
    ...
    Protected Sub btnLink_Click(ByVal sender As Object, ByVal e As EventArgs)
        Dim btnLink As LinkButton = DirectCast(sender, LinkButton)
        Dim row As GridViewRow = DirectCast(btnLink.NamingContainer, GridViewRow)
        Dim index As Integer = row.RowIndex

        ' Get the CommandName and CommandArgument from the clicked LinkButton
        Dim commandName As String = btnLink.CommandName
        Dim commandArgument As String = btnLink.CommandArgument

        ' Perform necessary actions based on the clicked LinkButton
    End Sub
