...
Partial Class estimate_new
    ...
    Protected Sub Button2_Click(sender As Object, e As System.EventArgs) Handles Button2.Click
        ...
        Else
            ...
            'Dim strnewrecord1 = "with q1 as (SELECT vehicle_parts.id as [Part ID], vehicle_parts.[part_no] as [Part No],[parts_name] as [Parts Name], [description] as [Description],((case when (SELECT sum(op.opening_qty) from vehicle_parts op where op.id=vehicle_parts.id  and datediff(day,op.opening_date,'" & DateTime.Now & "')>=0) is null then '0' else (SELECT sum(op.opening_qty) from vehicle_parts op where op.id=vehicle_parts.id  and datediff(day,op.opening_date,'" & DateTime.Now & "')>=0) end) + (case when (SELECT sum(pu.qty) from purchase_item pu where pu.part_id=vehicle_parts.id  and datediff(day,pu.date,'" & DateTime.Now & "')>=0) is null then '0' else (SELECT sum(pu.qty) from purchase_item pu where pu.part_id=vehicle_parts.id  and datediff(day,pu.date,'" & DateTime.Now & "')>=0) end) - (case when (SELECT sum(issue.qty) from job_wise_item_issue issue where issue.part_id=vehicle_parts.id  and datediff(day,issue.issue_date,'" & DateTime.Now & "')>=0) is null then '0' else (SELECT sum(issue.qty) from job_wise_item_issue issue where issue.part_id=vehicle_parts.id   and datediff(day,issue.issue_date,'" & DateTime.Now & "')>=0) end)) as [Available Quantity], limit,limito,limitr,alternet_parts  FROM vehicle_parts where ((case when (SELECT sum(op.opening_qty) from vehicle_parts op where op.id=vehicle_parts.id  and datediff(day,op.opening_date,'" & DateTime.Now & "')>=0) is null then '0' else (SELECT sum(op.opening_qty) from vehicle_parts op where op.id=vehicle_parts.id  and datediff(day,op.opening_date,'" & DateTime.Now & "')>=0) end) + (case when (SELECT sum(pu.qty) from purchase_item pu where pu.part_id=vehicle_parts.id   and datediff(day,pu.date,'" & DateTime.Now & "')>=0) is null then '0' else (SELECT sum(pu.qty) from purchase_item pu where pu.part_id=vehicle_parts.id    and datediff(day,pu.date,'" & DateTime.Now & "')>=0) end) - (case when (SELECT sum(issue.qty) from job_wise_item_issue issue where issue.part_id=vehicle_parts.id   and datediff(day,issue.issue_date,'" & DateTime.Now & "')>=0) is null then '0' else (SELECT sum(issue.qty) from job_wise_item_issue issue where issue.part_id=vehicle_parts.id    and datediff(day,issue.issue_date,'" & DateTime.Now & "')>=0) end))<=[limito]), q2 as (select q1.[alternet_parts] as [Part No1],q1.[alternet_parts] as [Part No],q1.[Parts Name],q1.[Description],sum(q1.[Available Quantity]) as [Available Quantity], q1.limit as [Limit],q1.limito as [limito],q1.limitr as [limitr] from q1 group by q1.[alternet_parts],q1.[Parts Name], q1.[Description],q1.limitr,q1.limito,q1.limit) "


            'strnewrecord = strnewrecord1 & "SELECT vehicle_parts.id as [Part ID], vehicle_parts.[part_no] as [Part No],[parts_name] as [Parts Name], [description] as [Description],'' as [Opening Quantity],'' as [Opening Value],'' as [Purchase Quantity],'' as [Purchase Value],'' as [Return Quantity],'' as [Return Value],'' as [Total Stock Quantity],'' as [Total Stock Value],'' as [Issued Quantity],'' as [Issued Value],'' as [Returned Quantity],'' as [Returned Value],'' as [Balance Quantity],'' as [Balance Value],'0' as [GRN No],(select sum(q2.[Available Quantity]) from q2 where q2.[Part No]=vehicle_parts.[alternet_parts]) as [Available Quantity], limit,limito,limitr FROM vehicle_parts where   (vehicle_parts.part_no like '%" & TextBox17.Text & "%' or vehicle_parts.alternet_parts like '%" & TextBox17.Text & "%' or  vehicle_parts.part_no in (select v.alternet_parts from vehicle_parts v where v.part_no like  '%" & TextBox17.Text & "%'))  order by vehicle_parts.part_no,vehicle_parts.parts_name, vehicle_parts.[description]"


            Dim str = " (SELECT ((case when (SELECT sum(op.opening_qty) from vehicle_parts op where op.part_no=vehicle_parts.part_no ) is null then '0' else"
            str &= " (SELECT sum(op.opening_qty) from vehicle_parts op where op.part_no=vehicle_parts.part_no) end) + (case when (SELECT sum(pu.qty) from purchase_item pu "
            str &= " where pu.part_no=vehicle_parts.part_no) is null then '0' else (SELECT sum(pu.qty) from purchase_item pu where pu.part_no=vehicle_parts.part_no) end) "
            str &= " - (case when (SELECT sum(issue.qty) from job_wise_item_issue issue where issue.part_no=vehicle_parts.part_no) Is null then '0' else "
            str &= " (SELECT sum(issue.qty) from job_wise_item_issue issue where issue.part_no=vehicle_parts.part_no ) end))) as qty "


            If TextBox18.Text <> "" Then
                Dim strnewrecord1 = "with q1 as (SELECT vehicle_parts.id as [Part ID], vehicle_parts.[part_no] as [Part No],[parts_name] as [Parts Name], [description] as [Description],((case when (SELECT sum(op.opening_qty) from vehicle_parts op where op.id=vehicle_parts.id  and datediff(day,op.opening_date,'" & DateTime.Now & "')>=0) is null then '0' else (SELECT sum(op.opening_qty) from vehicle_parts op where op.id=vehicle_parts.id  and datediff(day,op.opening_date,'" & DateTime.Now & "')>=0) end) + (case when (SELECT sum(pu.qty) from purchase_item pu where pu.part_id=vehicle_parts.id  and datediff(day,pu.date,'" & DateTime.Now & "')>=0) is null then '0' else (SELECT sum(pu.qty) from purchase_item pu where pu.part_id=vehicle_parts.id  and datediff(day,pu.date,'" & DateTime.Now & "')>=0) end) - (case when (SELECT sum(issue.qty) from job_wise_item_issue issue where issue.part_id=vehicle_parts.id  and datediff(day,issue.issue_date,'" & DateTime.Now & "')>=0) is null then '0' else (SELECT sum(issue.qty) from job_wise_item_issue issue where issue.part_id=vehicle_parts.id   and datediff(day,issue.issue_date,'" & DateTime.Now & "')>=0) end)) as [Available Quantity], limit,limito,limitr,alternet_parts  FROM vehicle_parts where ((case when (SELECT sum(op.opening_qty) from vehicle_parts op where op.id=vehicle_parts.id  and datediff(day,op.opening_date,'" & DateTime.Now & "')>=0) is null then '0' else (SELECT sum(op.opening_qty) from vehicle_parts op where op.id=vehicle_parts.id  and datediff(day,op.opening_date,'" & DateTime.Now & "')>=0) end) + (case when (SELECT sum(pu.qty) from purchase_item pu where pu.part_id=vehicle_parts.id   and datediff(day,pu.date,'" & DateTime.Now & "')>=0) is null then '0' else (SELECT sum(pu.qty) from purchase_item pu where pu.part_id=vehicle_parts.id    and datediff(day,pu.date,'" & DateTime.Now & "')>=0) end) - (case when (SELECT sum(issue.qty) from job_wise_item_issue issue where issue.part_id=vehicle_parts.id   and datediff(day,issue.issue_date,'" & DateTime.Now & "')>=0) is null then '0' else (SELECT sum(issue.qty) from job_wise_item_issue issue where issue.part_id=vehicle_parts.id    and datediff(day,issue.issue_date,'" & DateTime.Now & "')>=0) end))<=[limito]), q2 as (select q1.[alternet_parts] as [Part No1],q1.[alternet_parts] as [Part No],q1.[Parts Name],q1.[Description],sum(q1.[Available Quantity]) as [Available Quantity], q1.limit as [Limit],q1.limito as [limito],q1.limitr as [limitr] from q1 group by q1.[alternet_parts],q1.[Parts Name], q1.[Description],q1.limitr,q1.limito,q1.limit) "
                strnewrecord = strnewrecord1 & "SELECT vehicle_parts.id as [Service ID],'Parts' as [Service Category], vehicle_parts.[part_no] as [Part No],[parts_name] as [Service Type],[parts_name] as [Service Vehicle Type],(select top(1) saling_price from purchase_item where part_no=vehicle_parts.part_no ) as [Unit Service Charge],'Spare' as spare,part_no,0 as qty1," & str & " FROM vehicle_parts where   (vehicle_parts.parts_name like '%" & TextBox18.Text & "%' or vehicle_parts.alternet_parts like '%" & TextBox17.Text & "%' or  vehicle_parts.part_no in (select v.alternet_parts from vehicle_parts v where v.parts_name like  '%" & TextBox18.Text & "%'))  order by vehicle_parts.part_no,vehicle_parts.parts_name, vehicle_parts.[description]"
                genf.populate_grid(strnewrecord, GridView4)
            ElseIf TextBox17.Text <> "" Then
                Dim strnewrecord1 = "with q1 as (SELECT vehicle_parts.id as [Part ID], vehicle_parts.[part_no] as [Part No],[parts_name] as [Parts Name], [description] as [Description],((case when (SELECT sum(op.opening_qty) from vehicle_parts op where op.id=vehicle_parts.id  and datediff(day,op.opening_date,'" & DateTime.Now & "')>=0) is null then '0' else (SELECT sum(op.opening_qty) from vehicle_parts op where op.id=vehicle_parts.id  and datediff(day,op.opening_date,'" & DateTime.Now & "')>=0) end) + (case when (SELECT sum(pu.qty) from purchase_item pu where pu.part_id=vehicle_parts.id  and datediff(day,pu.date,'" & DateTime.Now & "')>=0) is null then '0' else (SELECT sum(pu.qty) from purchase_item pu where pu.part_id=vehicle_parts.id  and datediff(day,pu.date,'" & DateTime.Now & "')>=0) end) - (case when (SELECT sum(issue.qty) from job_wise_item_issue issue where issue.part_id=vehicle_parts.id  and datediff(day,issue.issue_date,'" & DateTime.Now & "')>=0) is null then '0' else (SELECT sum(issue.qty) from job_wise_item_issue issue where issue.part_id=vehicle_parts.id   and datediff(day,issue.issue_date,'" & DateTime.Now & "')>=0) end)) as [Available Quantity], limit,limito,limitr,alternet_parts  FROM vehicle_parts where ((case when (SELECT sum(op.opening_qty) from vehicle_parts op where op.id=vehicle_parts.id  and datediff(day,op.opening_date,'" & DateTime.Now & "')>=0) is null then '0' else (SELECT sum(op.opening_qty) from vehicle_parts op where op.id=vehicle_parts.id  and datediff(day,op.opening_date,'" & DateTime.Now & "')>=0) end) + (case when (SELECT sum(pu.qty) from purchase_item pu where pu.part_id=vehicle_parts.id   and datediff(day,pu.date,'" & DateTime.Now & "')>=0) is null then '0' else (SELECT sum(pu.qty) from purchase_item pu where pu.part_id=vehicle_parts.id    and datediff(day,pu.date,'" & DateTime.Now & "')>=0) end) - (case when (SELECT sum(issue.qty) from job_wise_item_issue issue where issue.part_id=vehicle_parts.id   and datediff(day,issue.issue_date,'" & DateTime.Now & "')>=0) is null then '0' else (SELECT sum(issue.qty) from job_wise_item_issue issue where issue.part_id=vehicle_parts.id    and datediff(day,issue.issue_date,'" & DateTime.Now & "')>=0) end))<=[limito]), q2 as (select q1.[alternet_parts] as [Part No1],q1.[alternet_parts] as [Part No],q1.[Parts Name],q1.[Description],sum(q1.[Available Quantity]) as [Available Quantity], q1.limit as [Limit],q1.limito as [limito],q1.limitr as [limitr] from q1 group by q1.[alternet_parts],q1.[Parts Name], q1.[Description],q1.limitr,q1.limito,q1.limit) "
                strnewrecord = strnewrecord1 & "SELECT vehicle_parts.id as [Service ID],'Parts' as [Service Category], vehicle_parts.[part_no] as [Part No],[parts_name] as [Service Type],[parts_name] as [Service Vehicle Type],(select top(1) saling_price from purchase_item where part_no=vehicle_parts.part_no ) as [Unit Service Charge],'Spare' as spare,part_no,0 as qty1," & str & " FROM vehicle_parts where   (vehicle_parts.part_no like '%" & TextBox17.Text & "%' or vehicle_parts.alternet_parts like '%" & TextBox17.Text & "%' or  vehicle_parts.part_no in (select v.alternet_parts from vehicle_parts v where v.part_no like  '%" & TextBox17.Text & "%'))  order by vehicle_parts.part_no,vehicle_parts.parts_name, vehicle_parts.[description]"
                genf.populate_grid(strnewrecord, GridView4)
            End If
    ...
    Protected Sub OrderCheckbox_Click(sender As Object, e As System.EventArgs)
        Dim list As New List(Of Int32)
        For k = 0 To GridView4.Rows.Count - 1

            GridView4.Rows(k).Cells(4).Text = genf.roundup(Val(GridView4.Rows(k).Cells(4).Text))
            GridView4.Rows(k).Cells(4).HorizontalAlign = HorizontalAlign.Right
            GridView4.Rows(k).Cells(0).Visible = False
            GridView4.Rows(k).Cells(1).Visible = False
            GridView4.Rows(k).Cells(4).Visible = False
            GridView4.Rows(k).Cells(2).Text = Server.HtmlDecode(GridView4.Rows(k).Cells(2).Text)
            GridView4.Rows(k).Cells(8).Visible = False
            Dim txt1, txt2 As New TextBox
            txt1 = GridView4.Rows(k).FindControl("TextBox1")
            txt2 = GridView4.Rows(k).FindControl("TextBox2")
            ...
            Else
                ...
                If GridView4.Rows(k).Cells(8).Text = "Service" Then
                    If Val(TextBox32.Text) > 0 Then
                        txt2.Text = Val(TextBox32.Text)
                    End If
                End If
                If GridView4.Rows(k).Cells(8).Text = "Spare" Then
                    If Val(TextBox35.Text) > 0 Then
                        txt2.Text = Val(TextBox35.Text)
                    End If
                End If

                If GridView4.Rows(k).Cells(8).Text = "Machine Shop" Then
            ...
            If GridView4.Rows(k).RowIndex = 0 Then
                txt1.Focus()
            End If

            'Dim drop As New DropDownList
            'drop = GridView4.Rows(k).FindControl("DropDownList5")
            ...
            'If GridView4.Rows(k).Cells(8).Text = "True" Then
            '    GridView4.Rows(k).Cells(8).Text = "Yes"
            'Else
            '    GridView4.Rows(k).Cells(8).Text = ""
            'End If



            GridView4.Rows(k).Cells(9).Text = Server.HtmlDecode(GridView4.Rows(k).Cells(9).Text)
            Dim orderCheckBox As CheckBox = GridView4.Rows(k).FindControl("OrderCheckbox")
        ...
        For k = 0 To GridView4.Rows.Count - 1
            GridView4.HeaderRow.Cells(0).Visible = False
            GridView4.HeaderRow.Cells(1).Visible = False
            GridView4.HeaderRow.Cells(4).Visible = False
        ...
        If list.Count > 0 Then
            ...
            For i = 0 To GridView4.Rows.Count - 1
                GridView4.Rows(i).Cells(11).Visible = True
                If list.Contains(i) Then
                    Dim partNoBoxCurrent As TextBox = GridView4.Rows(i).FindControl("PartNoBox")
                    partNoBoxCurrent.Enabled = True
                Else
                    Dim partNoBoxCurrent As TextBox = GridView4.Rows(i).FindControl("PartNoBox")
                    partNoBoxCurrent.Enabled = False
                End If
            Next
        End If
    ...
    Protected Sub GridView4_RowDataBound(sender As Object, e As System.Web.UI.WebControls.GridViewRowEventArgs) Handles GridView4.RowDataBound
        If e.Row.RowType = DataControlRowType.DataRow Then
            ...
            Dim orderCheckBox As CheckBox = e.Row.FindControl("OrderCheckbox")
    ...
    Protected Sub GridView4_SelectedIndexChanged(sender As Object, e As System.EventArgs) Handles GridView4.SelectedIndexChanged
        ...
        For i = 0 To GridView4.HeaderRow.Cells.Count - 2
            If i = 0 Then
                ...
                If i < GridView4.HeaderRow.Cells.Count - 1 Then
                    arcnt = arcnt + 1
                End If
            Else
        ...
        Dim txt3 As TextBox = GridView4.SelectedRow.FindControl("PartNoBox")
        drop = GridView4.SelectedRow.FindControl("DropDownList5")
        drop1 = GridView4.SelectedRow.FindControl("DropDownList1")
        Dim serviceId = GridView4.SelectedRow.Cells(0).Text
        ...
        For i = 0 To GridView4.SelectedRow.Cells.Count - 2
            If i = 0 Then
                ...
                If i <> 5 And i <> 6 And i <> 8 And i <> 7 And i <> 10 Then
            ...
            Else
                ...
                If GridView4.SelectedRow.Cells(i).Visible = True Then
                    If i <> 5 And i <> 6 And i <> 7 And i <> 8 And i <> 11 Then
                        arr1(cnt) = GridView4.SelectedRow.Cells(i).Text
                        'cnt = cnt + 1
                        flag = 1
                        flag3 = 1
                    Else
                        ...
                        End If
                        If i = 11 Then
                            arr1(cnt) = GridView4.SelectedRow.Cells(11).Text
                        End If
                        'If i = 8 Then
                        '    'arr1(cnt) = drop.SelectedValue.ToString
                        '    'cnt = cnt + 1
        ...
        arr1(5) = drop1.SelectedItem.Text
        arr1(10) = GridView4.SelectedRow.Cells(10).Text

        'If dt.Rows.Count = 0 Then
        '    Dim maxServiceId = genf.returnvaluefromdv("select max(service_id) from service_master", 0)
        '    arr1(0) = Val(maxServiceId) + 1
        'Else
        '    arr1(0) = Val(dt.Rows(dt.Rows.Count - 1)(0).ToString()) + 1
        'End If

        Myconnection = New System.Data.SqlClient.SqlConnection(strconnection)
        Myconnection.Open()
        Dim service_id
        strnewrecord = "INSERT INTO [service_master] ([service_category],[service_name],[car_type],[service_charge],spare) values ('" & arr1(1) & "','" & arr1(1) & "','" & arr1(1) & "','" & Val(arr1(6)) & "','" & arr1(8) & "'); select scope_identity()"
        Mycommand = New SqlCommand(strnewrecord, Myconnection)
        service_id = Mycommand.ExecuteScalar.ToString
        arr1(0) = Val(service_id)
        If flag3 = 1 Then
            dt.Rows.Add(arr1)
        End If
        ' dt.Rows.Add(arr)
        ...
        Dim iii = 0
        For Each row As GridViewRow In GridView5.Rows
            If row.Cells(2).Text = serviceId Then
                GridView5.DeleteRow(iii)
            End If
            iii = iii + 1
        Next
        TextBox41.Visible = True
    ...
    Protected Sub GridView5_SelectedIndexChanged(sender As Object, e As System.EventArgs) Handles GridView5.SelectedIndexChanged
        ...
        'Dim strnewrecord = "SELECT vehicle_parts.id as [Service ID],'Parts' as [Service Category], vehicle_parts.[part_no] as [Part No],[parts_name] as [Service Type],[parts_name] as [Service Vehicle Type],(select top(1) saling_price from purchase_item where part_no=vehicle_parts.part_no ) as [Unit Service Charge],'Spare' as spare,part_no,0 as qty1," & Str() & " FROM vehicle_parts where   (vehicle_parts.part_no like '%" & TextBox17.Text & "%' or vehicle_parts.alternet_parts like '%" & TextBox17.Text & "%' or  vehicle_parts.part_no in (select v.alternet_parts from vehicle_parts v where v.part_no like  '%" & TextBox17.Text & "%'))  order by vehicle_parts.part_no,vehicle_parts.parts_name, vehicle_parts.[description]"
        'genf.populate_grid(strnewrecord, GridView4)

        'genf.populate_grid("SELECT [service_id] as [Service ID],[service_category] as [Service Category],[service_name] as [Service Type],[car_type] as [Service Vehicle Type],[service_charge] as [Unit Service Charge], spare,'" & GridView5.SelectedRow.Cells(6).Text & "' as [Qty] FROM [service_master] where service_id='" & GridView5.SelectedRow.Cells(2).Text & "'", GridView4)
        genf.populate_grid("SELECT [service_id] as [Service ID],[service_category] as [Service Category],[service_name] as [Service Type],'" & GridView5.SelectedRow.Cells(12).Text & "' as [Part No],'" & GridView5.SelectedRow.Cells(12).Text & "' as part_no,[car_type] as [Service Vehicle Type],'" & GridView5.SelectedRow.Cells(5).Text & "' as [Unit Service Charge], spare,'" & GridView5.SelectedRow.Cells(6).Text & "' as [Qty1],'" & GridView5.SelectedRow.Cells(11).Text & "' as [Qty]  FROM [service_master] where service_id='" & GridView5.SelectedRow.Cells(2).Text & "'", GridView4)
        ...
        '        Dim str = " (SELECT ((case when (SELECT sum(op.opening_qty) from vehicle_parts op where op.part_no=e.part_no ) is null then '0' else"
        '        str &= " (SELECT sum(op.opening_qty) from vehicle_parts op where op.part_no=e.part_no) end) + (case when (SELECT sum(pu.qty) from purchase_item pu "
        '        str &= " where pu.part_no=e.part_no) is null then '0' else (SELECT sum(pu.qty) from purchase_item pu where pu.part_no=e.part_no) end) "
        '        str &= " - (case when (SELECT sum(issue.qty) from job_wise_item_issue issue where issue.part_no=e.part_no) Is null then '0' else "
        '        str &= " (SELECT sum(issue.qty) from job_wise_item_issue issue where issue.part_no=e.part_no ) end))) as qty "


        '        genf.populate_grid("SELECT [service_id] as [Service ID],[service_category] as [Service Category],[service_name] as [Service Type],[car_type] as [Service Vehicle Type],[service_charge] as [Unit Service Charge], spare ,part_no," & str & "  FROM [service_master] join vehicle_parts e on [service_master].service_name=e.parts_name  where service_id='" & Val(TextBox20.Text) & "' ", GridView4)
        ...
        '        'Dim orderCheckBox As CheckBox = GridView4.Rows(0).FindControl("OrderCheckbox")
        '        ''Dim PartNoBox As TextBox = GridView4.Rows(0).FindControl("PartNoBox")
        '        ''If Not IsNothing(Request.QueryString("url")) Then
        '        '    If GridView5.SelectedRow.Cells(12).Text <> "" Then
        '        '        isChecked = True
        '        '    Else
        '        '        isChecked = False
        '        '    End If
        '        'Else
        '        '    If GridView5.SelectedRow.Cells(11).Text <> "" Then
        '        '        isChecked = True
        '        '    Else
        '        '        isChecked = False
        '        '    End If
        ...
        '        'PartNoBox.Text = GridView5.SelectedRow.Cells(12).Text
        '        'GridView4.Rows(0).Cells(11).Visible = True
    ...
    Protected Sub Button4_Click(sender As Object, e As System.EventArgs) Handles Button4.Click
        ...
        strnewrecord = "delete from estimate_order_placement where estimate_job_id='" & Val(job_id.ToString) & "'"
        Mycommand = New SqlCommand(strnewrecord, Myconnection)
        Mycommand.ExecuteNonQuery()
        ...
        'For i = 0 To GridView5.Rows.Count - 1
        '    strnewrecord = "INSERT INTO [Operation_PartNumber]"
        '    strnewrecord = strnewrecord & "([job_id]"
        '    strnewrecord = strnewrecord & ",[service_id]"
        ...
        '    strnewrecord = strnewrecord & "('" & Val(job_id.ToString) & "'"
        '    strnewrecord = strnewrecord & ",'" & Val(GridView5.Rows(i).Cells(2).Text) & "'"
        '    strnewrecord = strnewrecord & ",'" & Val(GridView5.Rows(i).Cells(12).Text) & "')"
        '    Mycommand = New SqlCommand(strnewrecord, Myconnection)
        '    Mycommand.ExecuteNonQuery()
        'Next
        ...
        For i = 0 To GridView5.Rows.Count - 1
            ...
            If GridView5.Rows(i).Cells(10).Text = "Spare" Then
                Dim part_id As String = genf.returnvaluefromdv("select id from vehicle_parts where  part_no='" + GridView5.Rows(i).Cells(12).Text + "'", 0)
                Dim maxId = genf.returnvaluefromdv("Select max(id) from estimate_order_placement", 0)
                Dim Id = (Val(maxId) + 1).ToString()
                strnewrecord = "INSERT INTO [estimate_order_placement]"
                strnewrecord = strnewrecord & " (id,estimate_job_id"
                strnewrecord = strnewrecord & ",parts_id"
                strnewrecord = strnewrecord & ",PartName"
                strnewrecord = strnewrecord & ",service_id"
                strnewrecord = strnewrecord & ",order_request_qty"
                strnewrecord = strnewrecord & ",parts_no)"
                strnewrecord = strnewrecord & " values "
                strnewrecord = strnewrecord & "(" + Id + ",'" & Val(job_id.ToString) & "'"
                strnewrecord = strnewrecord & ",'" & part_id & "'"
                strnewrecord = strnewrecord & ",'" & GridView5.Rows(i).Cells(3).Text & "'"
                strnewrecord = strnewrecord & ",'" & Val(GridView5.Rows(i).Cells(2).Text) & "'"
                strnewrecord = strnewrecord & ",'" & Val(GridView5.Rows(i).Cells(6).Text) & "'"
                strnewrecord = strnewrecord & ",'" & GridView5.Rows(i).Cells(12).Text & "')"
    ...
    Protected Sub Button15_Click(sender As Object, e As System.EventArgs) Handles Button15.Click
        ...
        genf.populate_grid("SELECT [service_id] as [Service ID],[service_category] as [Service Category],[service_name] as [Service Type],[car_type] as [Service Vehicle Type],'" & Val(TextBox32.Text) & "'  as [Qty],'" & Val(TextBox33.Text) & "'  as [Unit Service Charge], '' as [Unit], spare,'" & TextBox17.Text & "' as part_no,'" & TextBox17.Text & "' as qty FROM [service_master] where service_id=" & Val(service_id.ToString) & "", GridView4)
        Myconnection.Close()
        TextBox18.Text = ""
        TextBox19.Text = ""
        TextBox17.Text = ""
                ...
                If GridView5.Rows.Count > 0 Then
                    For Each row As GridViewRow In GridView5.Rows
                        If row.RowIndex <> GridView5.SelectedIndex Then
                            ...
                            If flag2 = 1 Then
                                Dim copyArray = arr1.Take(10).ToArray()
                                dt.Rows.Add(arr1)
                            End If
                        End If
    ...
    Protected Sub ManualOrderCheckbox_Click(sender As Object, e As System.EventArgs)
        If ManualOrderCheckbox.Checked = True Then
            'TextBox41.Visible = True
            'Label64.Visible = True
        Else
    ...
    Protected Sub Button16_Click(sender As Object, e As System.EventArgs) Handles Button16.Click
        ...
        Myconnection.Open()
        Dim service_id
        strnewrecord = "INSERT INTO [service_master] ([service_category],[service_name],[car_type],[service_charge],spare) values ('" & TextBox34.Text.Replace("'", "''") & "','" & TextBox34.Text.Replace("'", "''") & "','" & TextBox19.Text.Replace("'", "''") & "','" & Val(TextBox33.Text) & "','Spare'); select scope_identity()"
        Mycommand = New SqlCommand(strnewrecord, Myconnection)
        service_id = Mycommand.ExecuteScalar.ToString
        'Mycommand.ExecuteNonQuery()
        ...
        Dim quantity = TextBox35.Text
        genf.populate_grid("SELECT [service_id] as [Service ID],'" + TextBox34.Text + "' as [Service Category],'" + TextBox34.Text + "' as [Service Type],'" + TextBox34.Text + "' as [Service Vehicle Type],'" & Val(TextBox35.Text) & "'  as [Qty],'" & Val(TextBox36.Text) & "'  as [Unit Service Charge], '" & DropDownList8.SelectedValue & "' as [Unit], spare,'" & TextBox41.Text & "' as part_no,'0' as qty FROM [service_master] where service_id=" & Val(service_id.ToString) & "", GridView4)
        Myconnection.Close()
        TextBox18.Text = ""
        TextBox19.Text = ""
        TextBox17.Text = ""
        ...
        Else
            If GridView4.Rows.Count = 1 Then
                ...
                If flag3 = 1 Then
                    ...
                    arr1(10) = arr1(9)
                    arr1(4) = quantity
                    arr1(5) = DropDownList8.SelectedValue
                    arr1(6) = Val(arr1(3)) * Val(quantity)
                    dt.Rows.Add(arr1)
                End If
                ' dt.Rows.Add(arr)
                ' Next
    ...
    Protected Sub Button17_Click(sender As Object, e As System.EventArgs) Handles Button17.Click
        ...
        genf.populate_grid("SELECT '" & service_id & "' as [Service ID],'Parts' as [Service Category],'' as [Part No],'" & TextBox37.Text.Trim() & "' as [Service Type],'" & TextBox37.Text.Trim() & "' as [Service Vehicle Type],'" & Val(TextBox39.Text) & "'  as [Unit Service Charge],'Machine Shop' as spare,'' as part_no,'" & Val(TextBox38.Text) & "' as [Qty1],0 as qty FROM [service_master] where service_id='" & Val(service_id.ToString) & "'", GridView4)
        ...
        Else
            If GridView4.Rows.Count = 1 Then
                    ...
                    If i = 0 Then
                        ...
                        Else
                            ...
                            If i = 7 Then
                                arr1(cnt) = drop1.SelectedItem.Text
                                cnt = cnt + 1
                                arr1(cnt) = Val(TextBox38.Text) * Val(txt1.Text)
                                ' cnt = cnt + 1
                                cnt = cnt + 1
                                arr1(cnt) = "-"
                                'cnt = cnt + 1
                ...
                arr1(5) = DropDownList9.SelectedItem.Text
                arr1(10) = TextBox41.Text
                ...
                TextBox41.Text = ""
                Label64.Text = ""
                ManualOrderCheckbox.Checked = False
                TextBox41.Visible = False
                GridView4.DataSource = Nothing
                GridView4.DataBind()
                TextBox18.Focus()
