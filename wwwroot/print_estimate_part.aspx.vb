...
Imports System.IO
Partial Class print_estimate_part
    ...
    Protected Sub GridView6_RowDataBound(sender As Object, e As System.Web.UI.WebControls.GridViewRowEventArgs) Handles GridView6.RowDataBound
        If e.Row.RowType = DataControlRowType.DataRow Then
            ...
            End If
            Dim partNo = genf.returnvaluefromdv("select service_id from service_master where service_name='" & e.Row.Cells(1).Text.Trim() & "' ", 0)
            Dim partName = ""
            If partNo <> "" Then
                partName = genf.returnvaluefromdv("select part_no from Operation_PartNumber where service_id=" & partNo & " and job_id=" & Val(Request.QueryString("url")) & " ", 0)

            End If
            e.Row.Cells(1).Text = Server.HtmlDecode("<div class='truncated' align='left'>&nbsp;" & e.Row.Cells(1).Text & " ( " & partName & " )</div>")
